...
public class TargetESP extends HackModule {
    ...
    public void renderHud(GuiGraphics ctx) {
        ...
        Vec3 left   = new Vec3(bb.minX - 0.1, target.getY() + target.getBbHeight()/2, target.getZ());
        Vec3 right  = new Vec3(bb.maxX + 0.1, target.getY() + target.getBbHeight()/2, target.getZ());
        ...
        MutableComponent info = Component.literal(name + " ")
            .append(Component.literal((int)hp + "/" + (int)maxHp).withStyle(s -> s.withColor(hpColor)))
            .append(Component.literal(" [" + dist + "m]").withStyle(s -> s.withColor(0xAAAAAA)));

        int tw = mc.font.width(info);
        ctx.fill((x1+x2)/2 - tw/2 - 2, y1 - 12,
                 (x1+x2)/2 + tw/2 + 2, y1 - 1, 0xAA000000);
        ctx.drawString(mc.font, info, (x1+x2)/2 - tw/2, y1 - 11, 0xFFFFFF, true);
